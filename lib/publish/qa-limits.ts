...
/** Per-IP burst limits. Deliberately tighter than the per-deck caps: one
 *  visitor should never be able to exhaust a deck's daily budget alone. */
export const IP_BURST_LIMIT = 5
export const IP_BURST_WINDOW_MS = 10 * 60 * 1000
...
export function evaluateLimits(counts: {
  ...
  if (counts.ipInWindow >= IP_BURST_LIMIT) {
    return {
      allowed: false,
      reason: 'rate_limited',
  ...
  if (counts.ipToday >= IP_DAILY_LIMIT) {
